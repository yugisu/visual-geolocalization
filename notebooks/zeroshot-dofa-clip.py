...
def _():
    ...
    # DOFA RGB wavelength metadata in micrometers (R, G, B).
    RGB_WAVELENGTHS_UM = [0.665, 0.56, 0.49]
    return (
        DEVICE,
        DataLoader,
        F,
...
@app.cell
def _(DEVICE, RGB_WAVELENGTHS_UM):
    from torchvision import transforms as T

    try:
        model = __import__("torch").hub.load(
            "zhu-xlab/DOFA",
            "vit_base_dofa",
            pretrained=True,
            trust_repo=True,
    ...
    # README uses NAIP mean/std (ImageNet stats in 0-255 space), so scale to [0,1] ToTensor output.
    ...
    embedder = model
    wave_list = RGB_WAVELENGTHS_UM
    return embedder, preprocess, wave_list
...
@app.cell(hide_code=True)
def _(DEVICE, DataLoader, F, embedder, time, torch, tqdm, wave_list):
    ...
    def extract_embeddings(loader: DataLoader) -> tuple[torch.Tensor, list[float], list[float], float]:
        ...
        for imgs, lats, lons in tqdm(loader, desc="Building embeddings"):
            imgs = imgs.to(DEVICE)
            embs = embedder.forward_features(imgs, wave_list=wave_list)
...
def _(
    ...
    entry = {
        "model": "earthflow/DOFA",
        "model_extra": {
            "variant": "vit_base_dofa",
            "mode": "DOFA-CLIP-style retrieval",
            "wave_list_um": RGB_WAVELENGTHS_UM,
        },
        "dataset": "visloc",
        "dataset_extra": {
            "flight_id": FLIGHT_ID,
...
def _(gallery_embeddings, np, project_root, query_embeddings):
    ...
    gallery_path = emb_dir / "zeroshot-dofa-clip-emb-gallery.npy"
    query_path = emb_dir / "zeroshot-dofa-clip-emb-query.npy"
