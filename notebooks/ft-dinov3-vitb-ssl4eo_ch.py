...
def _():
    ...
    from lib.full_dinov3_ft_backbone import DINO_MODEL, chamfer_rerank
    from lib.dinov3_pretrain_lora_backbone import load_ssl_backbone, DEFAULT_PRETRAIN_CHECKPOINT
    ...
    ckpt_path = ckpt_root / DEFAULT_PRETRAIN_CHECKPOINT
...
@app.cell(hide_code=True)
def _(DEVICE, DINO_MODEL, ckpt_path, load_ssl_backbone):
    from transformers import AutoImageProcessor

    processor = AutoImageProcessor.from_pretrained(DINO_MODEL)
    model = load_ssl_backbone(ckpt_path=str(ckpt_path), model_name=DINO_MODEL)
...
def _(DEVICE, DataLoader, F, embedder, time, torch, tqdm):
    ...
    def extract_embeddings(
        ...
        for imgs, lats, lons in tqdm(loader, desc="Building embeddings"):
            imgs = imgs.to(DEVICE)
            out = embedder(pixel_values=imgs).last_hidden_state
            cls = F.normalize(out[:, 0], dim=-1)

            if tta:
                r1 = F.normalize(embedder(pixel_values=torch.rot90(imgs, 1, [2, 3])).last_hidden_state[:, 0], dim=-1)
                r2 = F.normalize(embedder(pixel_values=torch.rot90(imgs, 2, [2, 3])).last_hidden_state[:, 0], dim=-1)
                r3 = F.normalize(embedder(pixel_values=torch.rot90(imgs, 3, [2, 3])).last_hidden_state[:, 0], dim=-1)
                cls = F.normalize((cls + r1 + r2 + r3) / 4.0, dim=-1)

            embeddings.append(cls.cpu())

            if with_patches:
                patches = F.normalize(out[:, 1:], dim=-1)
                patch_tokens.append(patches.cpu().half())
...
def _(
    ...
    entry = {
        "model": "ft-dinov3-vitb-ssl4eo_ch",
        "model_extra": {
            "TTA": TTA,
...
def _(
    ...
    base_name = "ft-dinov3-vitb-ssl4eo_ch"
