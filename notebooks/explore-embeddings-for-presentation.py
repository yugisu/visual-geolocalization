...
def _(
    ...
    return embedding_df, uav_coords
...
@app.cell
def _(gallery_dataset, plt, uav_dataset):
    _sat_img = gallery_dataset._img
    _h, _w = _sat_img.shape[:2]
    _lat_min, _lon_min, _lat_max, _lon_max = gallery_dataset._bounds

    _uav_df = uav_dataset.records.iloc[:5]
    _gt_x = ((_uav_df["lon"] - _lon_min) / (_lon_max - _lon_min) * _w).to_numpy(dtype=float)
    _gt_y = ((_lat_max - _uav_df["lat"]) / (_lat_max - _lat_min) * _h).to_numpy(dtype=float)
    ...
    _fig.savefig("fig-example-retrieval-map.png", dpi=500, bbox_inches="tight")
    return
...
@app.cell(hide_code=True)
def _(focus_query_idx, mo, np, plt, uav_dataset):
    mo.stop(focus_query_idx is None, mo.md("Select exactly one query point to view the cropped UAV image."))

    _q_img, _q_lat, _q_lon = uav_dataset[focus_query_idx]
    _q_arr = np.asarray(_q_img)
    _h, _w = _q_arr.shape[:2]
    _square_size = min(_h, _w)
    _y0 = (_h - _square_size) // 2
    _x0 = (_w - _square_size) // 2
    _q_square = _q_arr[_y0 : _y0 + _square_size, _x0 : _x0 + _square_size]

    _fig, _ax = plt.subplots(1, 1, figsize=(5, 5))
    _ax.imshow(_q_square)
    _ax.set_xticks([])
    _ax.set_yticks([])
    _ax.axis("off")
    _fig.tight_layout(pad=0)
    ...
    _fig.savefig("fig-example-retrieval-uav-img.png", dpi=300, bbox_inches="tight")
...
@app.cell
def _(focus_query_idx, gallery_dataset, mo, np, plt, pred_chunk_idx):
    mo.stop(focus_query_idx is None, mo.md("Select exactly one query point to view top-1 and random satellite chunks."))
    mo.stop(pred_chunk_idx is None, mo.md("No top-1 satellite chunk is available for the selected query."))

    _rng = np.random.default_rng(42 + int(focus_query_idx))
    _all_gallery_indices = np.arange(len(gallery_dataset), dtype=int)
    _random_pool = _all_gallery_indices[_all_gallery_indices != int(pred_chunk_idx)]
    _random_count = min(7, len(_random_pool))
    _random_indices = _rng.choice(_random_pool, size=_random_count, replace=False).astype(int).tolist()
    _grid_indices = [int(pred_chunk_idx), *_random_indices]

    _fig, _axes = plt.subplots(2, 4, figsize=(12.4, 6.4))
    _axes = np.atleast_1d(_axes).ravel()

    for _ax in _axes[len(_grid_indices) :]:
        _ax.axis("off")

    for _ax, _chunk_idx in zip(_axes, _grid_indices):
        _sat_img, _, _ = gallery_dataset[int(_chunk_idx)]
        _ax.imshow(np.asarray(_sat_img))
        _ax.set_xticks([])
        _ax.set_yticks([])
        _ax.axis("off")
    ...
    _fig.savefig("fig-example-retrieval-sat-imgs.png", dpi=300, bbox_inches="tight")
...
def _(
    ...
    _fig, _ax = plt.subplots(1, 1, figsize=(5, 5))
    _ax.imshow(gallery_dataset[pred_chunk_idx][0])
    _ax.set_xticks([])
    _ax.set_yticks([])
    _ax.axis("off")
    _fig.tight_layout(pad=0)
    ...
    _fig.savefig("fig-example-retrieval-sat-correct-img.png", dpi=300, bbox_inches="tight")
