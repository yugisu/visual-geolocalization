...
@app.cell
def _(DEVICE, project_root, torch, os, sys):
    import subprocess

    from huggingface_hub import hf_hub_download
    from torchvision import transforms as T

    satclip_root = project_root / "third_party" / "satclip"
    if not satclip_root.exists():
        satclip_root.parent.mkdir(parents=True, exist_ok=True)
        subprocess.run(
            ["git", "clone", "--depth", "1", "https://github.com/microsoft/satclip.git", str(satclip_root)],
            check=True,
        )

    sys.path.insert(0, str(satclip_root / "satclip"))

    from load import get_satclip

    SATCLIP_MODEL_ID = os.environ.get("SATCLIP_MODEL_ID", "microsoft/SatCLIP-ResNet50-L40")
    SATCLIP_CKPT_NAME = os.environ.get("SATCLIP_CKPT_NAME", "satclip-resnet50-l40.ckpt")

    ckpt_path = hf_hub_download(SATCLIP_MODEL_ID, SATCLIP_CKPT_NAME)

    model = get_satclip(ckpt_path, return_all=True, device=DEVICE)
    model = model.to(DEVICE).eval()

    target_size = getattr(model.visual, "input_resolution", 256)

    def to_13_channels(img):
        tensor = T.ToTensor()(img)
        if tensor.shape[0] == 3:
            pad = torch.zeros(10, tensor.shape[1], tensor.shape[2], dtype=tensor.dtype)
            tensor = torch.cat([tensor, pad], dim=0)
        return tensor

    preprocess = T.Compose([
        T.Resize((target_size, target_size)),
        T.Lambda(to_13_channels),
    ])

    embedder = model.visual
    return SATCLIP_CKPT_NAME, SATCLIP_MODEL_ID, embedder, preprocess
...
def _(
    ...
    entry = {
        "model": SATCLIP_MODEL_ID,
        "model_extra": {
            "checkpoint": SATCLIP_CKPT_NAME,
            "rgb_pad_channels": 10,
        },
        "dataset": "visloc",
        "dataset_extra": {
...
def _(gallery_embeddings, np, project_root, query_embeddings):
    ...
    gallery_path = emb_dir / "zeroshot-satclip-emb-gallery.npy"
    query_path = emb_dir / "zeroshot-satclip-emb-query.npy"
